...
- ⚠️ **分镜就是真实数据源**：核对分镜/旁白/配音/时长/镜组对位时，一律用 `shot get` / `shot list` 从途视读**实际分镜**（含 `DialogueName`/`Dialogue`/`Time`/产物路径）。**不要读 AI 自己生成的分镜表、旁白稿等文档或记忆来反推**——那些是生成时快照，AI 可能只凭自产文档作答而没碰途视里的真实分镜，从而拿错时长/对位
